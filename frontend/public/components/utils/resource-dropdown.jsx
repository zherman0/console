...
import { Firehose, LoadingInline, Dropdown, ResourceName } from '.';

/* Component ResourceDropdown - creates a dropdown list of resources specified */

class ResourceDropdown_ extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      items: {},
    ...
    this.onChange = key => {
      ...
      this.setState({
        selectedKey: key,
        title: <ResourceName kind={kindLabel} name={name} />,
      });
      this.props.onChange({ target: { value: name, name: this.props.name } });
    };
  }
  ...
  render() {
    ...
    _.each(
      this.state.items,
      (v, key) =>
        (items[key] = <ResourceName kind={v.kindLabel} name={v.name} />)
    );
      ...
      <div>
        ...
        {loaded &&
          ...
          <p className="alert alert-info">
            <span className="pficon pficon-info" aria-hidden="true" />
              No {desc} found or defined.
          </p>
        )}
      </div>
    );
