...
class AttachStorageForm extends React.Component<AttachStorageFormProps, AttachStorageFormState> {
  state = {
    resourceObj: null,
    inProgress: false,
    claimName: '',
  ...
  onMountAsReadOnlyChanged: React.ReactEventHandler<HTMLInputElement> = event => {
  ...
  save = (event: React.FormEvent<EventTarget>) => {
    ...
    const { resourceObj: originalObj, claimName, volumeName, mountPath, subPath, mountAsReadOnly, volumeAlreadyMounted } = this.state;
  ...
  checkMountPaths = (path: string) => {
    const existingMountPaths = this.getMountPaths(this.state.resourceObj.spec.template);
    const error = existingMountPaths.indexOf(path) > -1 ? 'Mount path is already in use.' : '';
    this.setState({ error });
  };

  render() {
    const { kindObj, name, namespace } = this.props;
    const { claimName, volumeName, volumeAlreadyMounted, mountPath, subPath, inProgress, error } = this.state;
    ...
    return (
      <div className="co-m-pane__body">
        ...
        >
          ...
          {kindObj && <div className="co-m-pane__explanation co-resource-link-wrapper">
            Add a persistent volume claim to&nbsp;<ResourceLink kind={kindObj.kind} name={name} namespace={namespace} />.
          </div>}
          ...
          <div className="form-group">
            ...
            <div>
              ...
              <p className="form-text text-muted" id="volume-name-help">
                Unique name to identfiy this volume.
              </p>
            </div>
          </div>
          <div className="form-group">
